...
export default function EditTransactionPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [accs, cats, srcs, txn] = await Promise.all([
        supabase
          .from("accounts")
          .select("*")
      ...
      if (txn.data) {
        const t = txn.data as Transaction;
        setType(t.type);
        setAmount(t.amount);
        setDescription(t.description || "");
        if (t.description) setShowDescInput(true);
        setDate(t.date);
        setAccountId(t.account_id);
        setToAccountId(t.to_account_id || "");
        setCategoryId(t.category_id || "");
        setIncomeSourceId(t.income_source_id || "");
      }
      setFetching(false);
    }
    loadData();
  ...
  async function handleSubmit() {
    if (amount <= 0) return toast.error("Masukkan nominal");
